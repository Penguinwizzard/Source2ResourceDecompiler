...
// conveniently, this coincides with the on-disk representation of the dmx files
// for a good number of these - allowing much faster parsing, since we just need
// to do a pointer update, instead of putting these in a separate structure and
// copying data around. The whole structure ends up being 7 bytes after packing,
// although the end is often ignored/unused.
typedef union __attribute__((__packed__)) _dmxcontentval_t {
	struct { // array
		uint32_t count;
		union _dmxcontentval_t
		    *array; // note that we treat this as a void* for
		            // directly-handled types
	};
	struct { // element
		int32_t index;
		union {
...
typedef struct {
	...
	dmxattribute *__indirectattributes; // Used for holding non-file-defined
	                                    // attributes after parse.
	...
	char type[4];
	int typeversion;
	// if using the threadsafe variants of the mutator/accessor functions,
	// we may need to lock sometimes.
