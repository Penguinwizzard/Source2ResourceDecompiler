...
#define READI( x ) *(x*)(*bufferptrptr); *bufferptrptr += sizeof(x);
// string handling
#define READISTRING() (char*)(*bufferptrptr); *bufferptrptr += strlen(*bufferptrptr)+1;
// object pointer handling
#define READIP( x ) (x*)(*bufferptrptr); *bufferptrptr += sizeof(x);
...
#define TYPECOUNT 16
	...
	switch(typeindex) {
		...
		case 0xF:; // byte
			 ...
			 if(array) {
				 ...
				 *bufferptrptr += lengths[typeindex]*count;
				 ...
				 dmxattribute* ret = (dmxattribute*)((*bufferptrptr)-sizeof(uint8_t));
		...
		case 0xC:; // quaternion
		case 0xD:; // matrix
			 // these are indirectly handled - we allocate space specially for these. 
			 dmxattribute* ret = allocindirectdmxattribute(dmx); 
			 ret->type = type;
				 ...
				 switch(typeindex) {
					 ...
					 case 0x4: // string
						 if(stringsraw) {
							 ...
							 writeto->strindex = READI(STRING_INDEX_SIZE);
						 }
						 break;
					 case 0x5: // byte[]
						 writeto->barr_length = READI(uint32_t);
						 writeto->barr_contents = (uint8_t*)*bufferptrptr;
						 //FILE* fp = fopen("dump.jpg","w");
						 //fwrite(writeto->barr_contents, sizeof(uint8_t), writeto->barr_length,fp);
						 //fclose(fp);
...
void V(printattribute) (const dmxattribute* attr) {
	if(attr == NULL) {
		fprintf(stderr,"Error: tried printing null pointer as dmx attribute\n");
		return;
	}
}

char* V(dictsearch) (char* const * const dict, const STRING_INDEX_SIZE index, const STRING_DICT_SIZE_TYPE count) {
...
#define READ( x ) *(x*)curindex; curindex += sizeof(x)
// string handling
#define READSTRING() (char*)curindex; curindex += strlen(curindex)+1;
// object pointer handling
#define READP( x ) (x*)(curindex); curindex += sizeof(x);
dmx* V(dmx_from_buffer) (char* buffer, const unsigned int length) {
	...
	// This is (I believe) how to properly parse it:
	{
		...
		if(curindex - buffer + sizeof(uint32_t) > length) {
			fprintf(stderr, "Error in dmx parse: insufficient data (type 0)\n");
			return NULL;
		}
		...
		for(uint32_t i=0;i<numelements;i++) {
			...
			if(i==0) {
				ret->prefixdata = (dmxattribute**)malloc(numattributes * sizeof(dmxattribute*));
			}
	...
	if(length < sizeof(STRING_DICT_SIZE_TYPE)) {
		fprintf(stderr, "Error in dmx parse: insufficient data (type 1)\n");
		return NULL;
	}
	...
	for(uint32_t i=0;i<numelements;i++) {
		...
		char* unknown = V(dictsearch) (strings,unknownindex,numstrings);
		GUID* guid = READP(GUID); 
		printf("ELEMENT_PARSE_TEST: %s: %s (%s) %hhx\n",type,name,unknown, guid->bytes[0]);
	...
	for(uint32_t elem=0;elem<numelements;elem++) {
		...
		for(uint32_t i=0;i<numattributes;i++) {
			...
			fflush(stdout);
			STRING_INDEX_SIZE attrnameindex = READ(STRING_INDEX_SIZE);
			printf("got index, it's %u\n",(uint32_t)attrnameindex);
			fflush(stdout);
			char* attrname = V(dictsearch) (strings,attrnameindex,numstrings);
...
char* V(dmx_to_buffer) (dmx* in, long* length) {
	...
	if(length == NULL) {
		fprintf(stderr, "Error: length return for dmx_to_buffer_%u was null!\n",DMXB);
		*length = 0;
		return NULL;
	}
	int size = 1000;
