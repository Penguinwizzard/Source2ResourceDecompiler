...
#define setOffsetString(a, b)                                                  \
	strncpy(pointer, (b), 64);                                             \
	(a) = pointer - ((char *)&(a));                                        \
	pointer += strlen(b) + 1;

char *construct_file(char *sourcefilename, char *sourcefilename2,
                     char *contentdirname, char *contents) {
	uint32_t length = strlen(sourcefilename) + strlen(contents) +
	                  strlen(contentdirname) + 400;
	...
	headers[1].tag[2] = 'T';
	headers[1].tag[3] = 'A';
	svfl_redi_header_datafile *srhd =
	    (svfl_redi_header_datafile *)(pointer);
	...
	svfl_redi_sourceresource_datafile *sourceref =
	    (svfl_redi_sourceresource_datafile *)pointer;
	pointer += srhd->sourceresource.count *
	           sizeof(svfl_redi_sourceresource_datafile);
	sourceref[0].CRC = 0; // Valve generally doesn't compile a vxml, and
	                      // this is just an alias, therefore the crc is 0
	sourceref[0].flags = 1;
	sourceref[1].CRC =
	    0; // This is generally non-zero, but it seems to be only
	       // used for checking when things need recompilation,
	       // which we're not bothering with (yet, anyway).
	sourceref[1].flags = 2;
	...
	// Creation arguments
	svfl_redi_argument_datafile *tdentry =
	    (svfl_redi_argument_datafile *)(pointer);
	...
	// Named data map
	svfl_redi_namemap_datafile *ndentry =
	    (svfl_redi_namemap_datafile *)pointer;
	...
	// Extra int data
	svfl_redi_extradata_int_datafile *sdentry =
	    (svfl_redi_extradata_int_datafile *)pointer;
	pointer += srhd->extradata_int.count *
	           sizeof(svfl_redi_extradata_int_datafile);
	sdentry->value = 0;
...
int main(int argc, char **argv) {
	if (argc < 6) {
		printf("Penguinwizzard Panorama Compiler v0.1\nUsage: "
		       "./panoramacompiler <filename> <outputfile> "
		       "<internalfilename> "
		       "<internalfilename2> <contentdirectory>\nThe last two "
		       "are used "
		       "inside the file for reference info.\n");
		return 1;
	}
	...
	if (num_ret != 1) {
		printf("Warning: File size is being wonky; expected 1 but got "
		       "%li\n",
		       num_ret);
	}
	fclose(f);
	...
	if (out == NULL) {
		printf("File open error; could not open output file; error "
		       "code is %i\n",
		       errno);
		return 1;
	}
